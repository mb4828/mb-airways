...
function setupSchedule(rawSchedule) {
    ...
    for (const f of FLIGHTS) {
        const tMin1 = {...f};
        tMin1.at = luxon.DateTime.fromJSDate(tMin1.at).plus(luxon.Duration.fromObject({day: -1})).toJSDate();
        tMin1.dt = luxon.DateTime.fromJSDate(tMin1.dt).plus(luxon.Duration.fromObject({day: -1})).toJSDate();
        flightsTMin1.push(tMin1);

        const tPlus1 = {...f};
        tPlus1.at = luxon.DateTime.fromJSDate(tPlus1.at).plus(luxon.Duration.fromObject({day: -1})).toJSDate();
        tPlus1.dt = luxon.DateTime.fromJSDate(tPlus1.dt).plus(luxon.Duration.fromObject({day: -1})).toJSDate();
